...
$venv = Join-Path $root ".venv-build-lite"
...
& $py -m pip install -r requirements-lite.txt pyinstaller

$dist = Join-Path $root "dist\\lite"
$build = Join-Path $root "build\\lite"
...
& $py -m PyInstaller --noconfirm --clean --distpath $dist --workpath $build (Join-Path $root "scripts\\workspace_brain_lite.spec")
...
Write-Host ""
Write-Host "[완료] lite 빌드 결과: $dist\\Workspace-Brain" -ForegroundColor Green
